...
router.put('/:id', checkPermission('tasks:write'), asyncHandler(async (req, res) => {
  const template = await templateService.updateTemplate(req.params.id as string, req.body, (req as any).user._id);
  res.json({ success: true, data: template, serverTime: new Date().toISOString() });
}));

router.delete('/:id', checkPermission('tasks:delete'), asyncHandler(async (req, res) => {
  const result = await templateService.deleteTemplate(req.params.id as string, (req as any).user._id);
...
router.post('/:id/generate', checkPermission('tasks:write'), asyncHandler(async (req, res) => {
  const task = await templateService.generateTasksFromTemplate(req.params.id as string, req.body.assigneeId, (req as any).user._id);
